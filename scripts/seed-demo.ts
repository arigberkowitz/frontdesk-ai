/**
 * Demo data seed for the first client (Bright Smile Dental).
 * Populates a full month of realistic calls / appointments / leads so the
 * dashboard, portal, and the appointments calendar look like a working practice.
 * Re-runnable: clears its own demo-tagged rows first (retell_call_id /
 * external_booking_id prefixed "demo_").
...
type Spec = {
  ...
  transcript: string;
  summary: string;
  // appt.inDays: + = future, − = earlier this month (relative to today).
...
const SPECS: Spec[] = [
  // ---- appointments spread across the month (earlier this month → upcoming) ----
  ...
  { daysAgo: 3, hour: 15, durSec: 101, from: "+14155550106", outcome: "booked", sentiment: "positive", cost: 14,
    transcript: "Caller: Need a cleaning for my daughter.\nAgent: Sure, let's find a time.", summary: "Booked a cleaning.",
    appt: { name: "Derek Lowe", inDays: 3, hour: 11, status: "booked", service: "Cleaning" } },
  ...
  { daysAgo: 5, hour: 14, durSec: 116, from: "+14155550108", outcome: "booked", sentiment: "positive", cost: 16,
    transcript: "Caller: Cleaning please, next week works.\nAgent: Booked for next week.", summary: "Booked a cleaning for next week.",
    appt: { name: "Omar Haddad", inDays: 7, hour: 10, status: "booked", service: "Cleaning" } },
  ...
  { daysAgo: 7, hour: 16, min: 30, durSec: 105, from: "+14155550111", outcome: "booked", sentiment: "positive", cost: 14,
    transcript: "Caller: Routine cleaning, mid-month if possible.\nAgent: Done — confirmed.", summary: "Cleaning booked mid-month.",
    appt: { name: "Liam Walsh", inDays: 10, hour: 15, status: "confirmed", service: "Cleaning" } },
  ...
  { daysAgo: 0, hour: 13, durSec: 99, from: "+14155550113", outcome: "booked", sentiment: "positive", cost: 13,
    transcript: "Caller: Can I get a cleaning later this month?\nAgent: Sure, booking you.", summary: "Cleaning booked later this month.",
    appt: { name: "Carlos Mendez", inDays: 16, hour: 14, status: "booked", service: "Cleaning" } },

  // ---- non-booking calls (variety) ----
  { daysAgo: 2, hour: 16, min: 40, durSec: 118, from: "+14155550176", outcome: "lead", sentiment: "neutral", cost: 14,
    transcript: "Caller: Do you do Invisalign? What's the cost?\nAgent: Pricing needs a consult — can I take your details for a callback?\nCaller: Sure, it's Priya.",
  ...
  { daysAgo: 3, hour: 19, min: 5, durSec: 70, from: "+14155550110", outcome: "faq_answered", sentiment: "positive", afterHours: true, cost: 8,
    transcript: "Caller: Are you open right now? What are your Saturday hours?\nAgent: We're closed at the moment — Mon–Fri 9 to 5, Saturdays 9 to 1.",
    summary: "After-hours caller — answered hours question (Sat 9–1)." },
...
async function main(): Promise<void> {
  const [client] = await sql<{ id: string; name: string }[]>`
    select id, name from clients where deleted_at is null order by created_at limit 1`;
  if (!client) throw new Error("No client found to seed.");
  const CLIENT_ID = client.id;

  const services = await sql<{ id: string; name: string; duration_min: number }[]>`
    select id, name, duration_min from services where client_id=${CLIENT_ID} and deleted_at is null order by created_at`;
  ...
  await sql`delete from appointments where client_id=${CLIENT_ID} and external_booking_id like 'demo_%'`;
  await sql`delete from leads where client_id=${CLIENT_ID}`;
  await sql`delete from calls where client_id=${CLIENT_ID} and retell_call_id like 'demo_%'`;
  ...
  for (const c of SPECS) {
    ...
    const [row] = await sql<{ id: string }[]>`
      ...
      values
        (${CLIENT_ID}, ${"demo_" + (1000 + i)}, 'inbound', ${c.from}, '+14155551234', ${start}, ${end},
         ${c.durSec}, ${c.transcript}, ${c.summary}, ${c.sentiment}::call_sentiment, ${c.outcome}::call_outcome,
         ${c.afterHours ?? false}, ${c.cost})
      returning id`;

    if (c.appt) {
      const svc = pickService(c.appt.service);
      ...
      await sql`
        ...
        values
          (${CLIENT_ID}, ${row.id}, ${c.appt.name}, ${c.from}, ${svc?.id ?? null},
           ${aStart}, ${aEnd}, ${c.appt.status}::appointment_status, ${"demo_cal_" + (1000 + i)})`;
    }

    if (c.lead) {
      await sql`
        insert into leads (client_id, call_id, name, phone, reason, message, status)
        values (${CLIENT_ID}, ${row.id}, ${c.lead.name}, ${c.from}, ${c.lead.reason}, ${c.lead.message}, ${c.lead.status}::lead_status)`;
    }
    i++;
  }
  ...
  const [counts] = await sql<{ calls: number; appts: number; leads: number }[]>`
    select
      (select count(*) from calls where client_id=${CLIENT_ID}) as calls,
      (select count(*) from appointments where client_id=${CLIENT_ID}) as appts,
      (select count(*) from leads where client_id=${CLIENT_ID}) as leads`;
  console.log(`Seeded ${client.name}:`, counts);
  await sql.end();
}
