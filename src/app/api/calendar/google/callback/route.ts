...
export async function GET(req: Request): Promise<Response> {
  ...
  const back = (status: string) => {
    ...
    // Clear the one-time CSRF cookie on the way out.
    res.cookies.set(OAUTH_STATE_COOKIE, "", { path: OAUTH_COOKIE_PATH, maxAge: 0 });
    return res;
  };

  // CSRF: the nonce in `state` must match the one-time cookie set when the flow started.
  ...
  if (!nonceOk) {
    logger.warn("calendar.google.state_mismatch", { clientId });
    return back("error");
  }
  ...
  try {
    ...
    logger.error("calendar.google.callback_failed", {
      clientId,
